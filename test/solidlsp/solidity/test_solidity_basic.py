...
class TestSolidityLanguageServer:
    ...
    def test_find_referencing_symbols(self, language_server: SolidLanguageServer) -> None:
        file_path = os.path.join("DemoContract.sol")
        ...
        for sym in symbols[0]:
            if sym.get("name") == "helperCheckBalance":
                helper_symbol = sym
                break
        assert helper_symbol is not None, "Could not find 'helperCheckBalance' function symbol in DemoContract.sol"
        sel_start = helper_symbol["selectionRange"]["start"]
        refs = language_server.request_references(file_path, sel_start["line"], sel_start["character"])
        assert any("DemoContract.sol" in ref.get("uri", "") for ref in refs), "Expected at least one reference result to point at DemoContract.sol"
